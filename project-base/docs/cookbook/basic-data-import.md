# Basic Data Import
This cookbook will guide you through process of importing data into your e-shop from external source,
e.g. from information system.
...
Let us create simple products import from external source.
...
We need to store relation between application database and external source of data because later, in data transfer processing, 
...
*Note: Cron modules are not suitable for data transfers initialized by external source,
...
In this step, we will download data from external source 
...
If a product exists with given `$externalId`, we modify it, otherwise we need to create new one. 
```php
...
*Note: We need to know whether product with given `$externalId` exists. 
...
that can talk to persistence layer. So we will implement new methods in these classes in next two steps.* 

#### 3.2 - Implement method `findByexternalId()` in [`ProductRepository`](../../src/Shopsys/ShopBundle/Model/Product/ProductRepository.php) in order to be able find [`Product`](../../src/Shopsys/ShopBundle/Model/Product/Product.php) by external ID
```php
...
and it's methods `create()` and `edit()`. 
Those methods expect [`ProductEditData`](../../src/Shopsys/ShopBundle/Model/Product/ProductEditData.php)
class as parameter, that you can create using
[`ProductEditDataFactory`](../../src/Shopsys/ShopBundle/Model/Product/ProductEditDataFactory.php).
...
```
*Note: In order to be able to use stock quantity, we must enable it by setting `$usingStock` attribute to `true`.*
...
**Warning: The method throws exception when [`Vat`](../../src/Shopsys/ShopBundle/Model/Pricing/Vat/Vat.php) object is not found by given percent value.
...
- Validate all incoming data before putting them into data objects.
    - You will avoid SQL errors (e.g. when incoming text is longer than db column allows).
- Transfer modifications only (if possible).
    - Persist the last successful transfer start datetime so next time you can import changes done afterwards.
    - It means less data to process.
- Transfer deleted entities too.
    - It is often omitted, but it is necessary if you want to keep consistency between application db and external source.
- Use db transactions.
    - Use `EntityManger` methods `beginTransaction()`, `commit()` and `rollback()`.
    - Do not forget to clear identity map when doing rollback (so even entity modifications are reverted).
...
- Disable entity manager SQL logging (use [`SqlLoggerFacade::temporarilyDisableLogging()`](../../src/Shopsys/ShopBundle/Component/Doctrine/SqlLoggerFacade.php)).
    - By default, every executed SQL query is logged and that slows down the process and consumes lot of memory when there are many iterations.
...
but it is not good idea to log "everything". Too many information in logs might be counterproductive.
...
for changes in all mapped entities and after time it consumes a huge amount of resources to persist a single entity.
    ...
    - You should load any entity again after clearing identity map, because any attempt to flush the old one will result in exception.
- Use streamed input for XML and JSON.
    - So you do not load a huge files at once (can lead to memory overflow).
...
- Restrict editing of the transferred fields in administration.
    - At least, mark them as transferred to avoid confusion when administrator changes the field value and then data import overrides the value.
...
- Be careful with the order of your data transfers.
    - For example, products have association with their categories,
...
- When processing large amount of data, you can use native queries instead of using ORM, which might save you a certain amount of SQL queries.
...
Now you know how to implement simple data transfer to your e-shop from external source.
