		...
		$productForm.jsFormValidator({
			'groups': function () {
				...
				if ($('input[name="product_edit[productData][usingStock]"]:checked').val() === '1') {
					...
					if($('select[name="product_edit[productData][outOfStockAction]"]').val() === SS6.constant('\\SS6\\ShopBundle\\Model\\Product\\Product::OUT_OF_STOCK_ACTION_SET_ALTERNATE_AVAILABILITY')) {
