	...
	$(document).ready(function () {
		...
		$productEditForm.jsFormValidator({
			'groups': function () {
				...
				if ($('input[name="product_edit_form[productData][priceCalculationType]"]:checked').val() === SS6.constant('\\SS6\\ShopBundle\\Model\\Product\\Product::PRICE_CALCULATION_TYPE_MANUAL')) {
		...
		$productForm.jsFormValidator({
			'groups': function () {
				...
				if ($('input[name="product_edit_form[productData][usingStock]"]:checked').val() === '1') {
					...
					if($('select[name="product_edit_form[productData][outOfStockAction]"]').val() === SS6.constant('\\SS6\\ShopBundle\\Model\\Product\\Product::OUT_OF_STOCK_ACTION_SET_ALTERNATE_AVAILABILITY')) {
				...
				if ($('input[name="product_edit_form[productData][priceCalculationType]"]:checked').val() === SS6.constant('\\SS6\\ShopBundle\\Model\\Product\\Product::PRICE_CALCULATION_TYPE_AUTO')) {
